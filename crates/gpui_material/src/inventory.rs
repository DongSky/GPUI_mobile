...
pub const INVENTORY: &[ComponentEntry] = &[
    ...
    ComponentEntry {
        ...
        parity: Parity::Done,
        notes: "WideNavigationRailItem Top/Start (Compose iconPositionFor): collapsed Top 56×32 + label-medium, expanded Start 56dp full-width pill + label-large; live Top→Start iconPosition lerp (spatial-fast); wide collapsed 96 / live narrow modal 80↔220; in-flow standard WideNavigationRail 96↔220 no scrim CornerNone/Surface + Extended FAB Create (56↔188); expanded 220–360 modal overlay 96 collapsed + 32% scrim; modalExpandedShape CornerLarge 16 + ModalContainerColor SurfaceContainer (spatial-fast lerp); ContentPadding 0/44/0/44 (WNRVerticalPadding = TopSpace); dismissible modal hideOnCollapse slides offscreen (Menu ☰, Start items, Arrangement.Center, expanded modal shape, always-extended Create ~188); header slot Menu/MenuOpen + Extended FAB Create (56↔188) + plain tooltip Above + Arrangement.Bottom; header-less modal/narrow Extended FAB Create (56↔188); active label secondary; OsPopupSpec PopUp not opened on Linux/NativeActivity; badges",
    },
    ComponentEntry {
        name: "Dialog",
    ...
    ComponentEntry {
        ...
        parity: Parity::Done,
        notes: "Modal calendar; 40dp days; selected/today/in-range/out-of-month; SelectableDates greys out Sat/Sun (Disabled, not tappable); Sunday-first grid; WeekDays BodyLarge / OnSurface + 48dp RecommendedSizeForAccessibility row min-height; DatePickerModalTokens.ContainerWidth 360 / ContainerHeight 568; DatePickerModalTokens.HeaderContainerHeight 120; DateEntryContainer header HorizontalDivider (under title/headline/toggle, DividerTokens.Color / Thickness 1; docked omitted); RangeSelectionHeaderContainerHeight 128 − HeaderHeightOffset 60; DatePickerTitlePadding / DatePickerHeadlinePadding (start 24 / end 12 / top 16 / bottom 12); DatePickerModeTogglePadding (end 12 / bottom 12) + DisplayModeToggleButton a11y (Switch to text input mode / Switch to calendar input mode); DateRangePickerTitlePadding / DateRangePickerHeadlinePadding (start 64 / end 12 / bottom 12) + range-header chrome (leading close/back in the 64dp start slot); DatePickerDialog DialogButtonsPadding (end 6 / bottom 8) + 8dp main-axis / 8dp cross-axis spacing; MonthsNavigation MonthYearHeight 56 + 48dp prev/next at end + MonthsNavigation a11y (Change to previous/next month; Switch to selecting a year / Swipe to select a year, or tap to switch back to selecting a day); range hero + DateRangePickerTitle Picker empty (Select dates + unselected VerticalMonthsList) + live start→end taps (Compose DateRangePicker: start, then end ≥ start, third tap restarts) + range-hero month nav (prev/next, YearRange clamp, cross-month InRange) + range-hero YearPicker (month ▾, 3×72×36, independent of the single-date modal) + range-hero showModeToggle (Picker calendar ↔ Input Start/End fields; range-input sibling stays) + range-hero Confirm/Cancel (draft until OK, header divider) + drawRangeBackground half-cell start/end connectors + range-hero VerticalMonthsList (2-month window, titleSmall month subhead, CalendarMonthSubheadPadding 24/20/8) + modal Confirm/Cancel (draft until OK, header divider; docked still commits on select) + modal month nav (prev/next, YearRange clamp, independent of the range hero) + docked popup (shadow, trailing DateRange icon, live day select writes field + dismisses, outside-click dismiss, month nav that re-grids HTML+GPUI, month ▾ YearPicker independent of the modal / range hero); modal date input (DisplayMode.Input, outlined MM/DD/YYYY, calendar/edit toggle, InputTextFieldPadding 24/24/10 + bottom 16) + DatePickerHeadline Picker empty (Selected date) + DatePickerHeadline Input empty (Entered date) + DateInputValidator supporting-text errors (Date format not recognized / Date out of expected year range 1900 - 2100 / Date not allowed via SelectableDates weekends); modal date range input (DateRangePicker DisplayMode.Input, Start/End outlined MM/DD/YYYY) + DateRangePickerHeadline empty (Start date – End date) + DateRangePickerHeadline start-only (Sep 15 – End date) + DateRangePickerHeadline end-only (Start date – Sep 21) + DateInputValidator supporting-text errors (Date format not recognized / Date out of expected year range 1900 - 2100 / Date not allowed via SelectableDates weekends / End date can't be before start date); live Picker↔Input showModeToggle on the modal; YearPicker (3×72×36, YearRange 1900–2100, month ▾ opens year pane) + YearPicker a11y (Year picker visible + Navigate to year) + YearPicker trailing HorizontalDivider; month ▾ chrome; emphasized large date",
    },
    ComponentEntry {
        name: "Search",
        ...
        parity: Parity::Done,
        notes: "Expressive contained (recommended): persistent surface-container-high when collapsed or docked, no divider, 56dp header + suggestion list; compact <600dp fullscreen ExpandedFullScreenSearchBar (0 margin / 0 corner) uses SearchBarDefaults.fullScreenContainedSearchBarColor surfaceContainerLow; medium ≥600dp ExpandedDockedSearchBar Corner 28 + 24→12dp; docked container min 240 / max ⅔ screen + 32% scrim over main content (scrim dismiss); results list scrolls beneath the bar; suggestion groups (Recent / Suggestions) separated by 8dp gaps; segmented filled rows (list 2dp gap, 4dp inner / 16dp outer, selected secondary-container); queried Quick results / Results use two-line 72dp rows (bodyMedium supporting, 40dp leading avatar / 20dp icon) and Results adds a trailing open affordance; FilterChips (All / Apps / Shortcuts / Settings) narrow queried results; queried search uses Quick results while focused and a Results label after submit (query visible, not focused, aria-live); trailing clear-X (✕) replaces mic when the query is non-empty (click empties + refocus); no-results empty state (No matching apps, 56dp, 0 results live region) when the filter returns nothing; divided full-screen activity + divider still available (spatial-fast height/corners/inset/scale, MorphLayerTransform top-center origin, with_animation PathBuilder::scale fill + morph_layer_box layout); caret editor + filtered suggestions; HTML morph container (no display:none swap)",
    },
    ComponentEntry {
        name: "Time picker",
        ...
        parity: Parity::Done,
        notes: "Expressive TimeScroll (recommended): two ScrollFields (12h or 00–23 / 00–59 wrap, 200dp / 3-item, ScrollFieldDefaults.shape / TimePickerDefaults.shapes().timeFieldShape CornerLarge 16) + vibrantColors primaryContainer fields inside TimePickerDialogDefaults.vibrantContainerColor surfaceContainer / vibrantShape CornerExtraLarge; catalog rAF + host vsync fling. TimeInput 96×72 CornerLarge fields + vibrant field outline (SurfaceContainerLowest / Primary 2dp focus / Transparent 1dp idle) + ScrollDisplayModeToggle (⌨/⇅ SwipeVertical Scroll↔Input) + TimePickerDialogDefaults.DisplayModeToggle a11y (Switch to text input mode / Switch to scroll mode / Switch to clock mode) + tooltip + DisplaySeparatorWidth 24 + PeriodToggleMargin 12 + Hour/Minute supporting text (SupportLabelTop 7). TimePickerDialogDefaults.Title: Picker/Scroll Select time, Input Enter time, padding bottom 20 + labelMedium. TimePickerCustomLayout portrait title top 24 / actions bottom 24 + Cancel/OK; landscape title 24 / content top 16 / actions bottom 8. 24-hour (is24Hour) uses 00–23 and hides AM/PM. Dial: Compose ClockFace 24h dual ring (outer 00–11 OuterCircle 101 / inner 12–23 InnerCircle 69) + 12h outer 1–12; ClockFaceSizeModifier TimePickerMaxHeight 384 → ClockDialContainerSize 256 / TimePickerMidHeight 330 → ClockDialMidContainerSize 238 / else ClockDialMinContainerSize 200 (compact hosts use Min; selector handle stays 48); TimeSelector selected PrimaryContainer / OnPrimaryContainer, idle SurfaceContainerHighest / OnSurface; PeriodSelector 1dp Outline shell (52×80 vertical / 216×38 horizontal / TimeInput 52×72, CornerSmall 8, no item gap) + PeriodToggle a11y (Select AM or PM) + TimeSelector a11y (Select hour / Select minutes) + TimeInput field a11y (for hour / for minutes) + ClockFace number a11y (TimePickerHourSuffix o'clock / TimePickerMinuteSuffix minutes / TimePicker24HourSuffix hours); ClockDisplayBottomMargin 36 + ClockFaceBottomMargin 24 on Vertical; DisplaySeparatorWidth 24 + PeriodToggleMargin 12; TimePickerLayoutType Vertical (compact) + Horizontal landscape (selectors beside 256dp ClockFace, 216×38 period, 24dp gap); analog hand spatial-fast lerp to ring radius; time selector 114dp (24h) / 96dp; wall-clock second hand at 16ms GPUI frames; displaySmallEmphasized header",
    },
    ComponentEntry {
        name: "Carousel",
