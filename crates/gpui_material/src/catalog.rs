...
.search-morph .sv-group {{ display: flex; flex-direction: column; gap: 2px; padding: 0 8px; }}
...
.search-morph[data-search-status="quick-results"] .sv-list,
.search-morph[data-search-status="results"] .sv-list {{
  gap: 2px;
}}
.search-morph[data-search-status="quick-results"] .sv-group,
.search-morph[data-search-status="results"] .sv-group {{
  padding: 0 8px; gap: 2px;
}}
.search-view .sv-row, .search-morph .sv-row {{
  display: flex; align-items: center; gap: 16px; padding: 0 16px;
  min-height: 56px; font-size: 16px; box-sizing: border-box;
}}
.search-morph .sv-row[data-search-row="segmented"] {{
  border-radius: 16px;
}}
.search-morph .sv-row[data-search-lines="two"] {{
  min-height: 72px; padding: 8px 16px; gap: 12px;
}}
.search-morph .sv-lead {{
  flex: 0 0 auto; display: flex; align-items: center; justify-content: center;
}}
.search-morph .sv-lead[data-search-leading="icon"] {{
  width: 20px; height: 20px; font-size: 20px; line-height: 20px;
}}
.search-morph .sv-lead[data-search-leading="avatar"] {{
  width: 40px; height: 40px; border-radius: 20px; font-size: 16px; font-weight: 500;
}}
...
.month-nav {{
  display: flex; align-items: center; justify-content: space-between;
  height: 56px; min-height: 56px; padding: 0; font-weight: 500;
}}
.month-nav-arrows {{ display: flex; align-items: center; }}
.month-nav button {{
  width: 48px; height: 48px; border: none; background: transparent;
  cursor: pointer; font-size: 18px; padding: 0; color: inherit;
  display: flex; align-items: center; justify-content: center;
}}
...
.cal {{ width: 360px; max-width: 360px; max-height: 568px; padding: 16px 12px 12px; }}
.cal .head {{
  margin: -16px -12px 0;
  padding: 0;
}}
.cal [data-date-header-min] {{ min-height: 120px; box-sizing: border-box; }}
.cal [data-date-range-header-min] {{ min-height: 68px; box-sizing: border-box; }}
.cal [data-date-title-pad] {{ padding: 16px 12px 0 24px; }}
.cal [data-date-headline-pad] {{ padding: 0 12px 12px 24px; }}
.cal [data-date-range-title-pad] {{ padding: 0px 12px 0 64px; }}
.cal [data-date-range-headline-pad] {{ padding: 0 12px 12px 64px; }}
.cal [data-range-header-chrome] {{ position: relative; }}
.cal [data-range-header-close] {{
  position: absolute; left: 8px; top: 8px; width: 48px; height: 48px;
  border: none; background: transparent; padding: 0; cursor: pointer;
  display: flex; align-items: center; justify-content: center;
  font-size: 24px; line-height: 24px; color: inherit;
}}
.cal .head .dp-toggle {{ margin-top: 16px; }}
.cal [data-date-range-header-paddings] .dp-toggle {{ margin-top: 0; }}
.cal [data-date-toggle-pad] {{ padding: 0 12px 12px 0; }}
.cal [data-date-dialog-buttons] {{
  margin: 0 -12px -12px 0;
  padding: 0 6px 8px 0;
  display: flex; flex-wrap: wrap; justify-content: flex-end;
  column-gap: 8px; row-gap: 8px;
}}
.cal .dp-entry-divider {{ height: 1px; margin: 0 -12px; }}
.cal .week, .cal .grid {{ display: grid; grid-template-columns: repeat(7, 40px); justify-content: center; }}
.cal .week[data-date-weekdays] {{ min-height: 48px; }}
.cal .week .day[data-date-weekday] {{ min-height: 48px; height: 48px; }}
...
.cal[data-date-pane="year"] .month-nav-arrows,
[data-datepicker-docked][data-docked-pane="year"] .month-nav-arrows {{ display: none; }}
.cal[data-date-pane="year"] .month-nav,
[data-datepicker-docked][data-docked-pane="year"] .month-nav {{ justify-content: flex-start; }}
.cal[data-date-pane="calendar"] .dp-years {{ display: none; }}
.cal[data-date-display="input"] .dp-years {{ display: none; }}
.cal .dp-year-divider,
[data-datepicker-docked] .dp-year-divider {{
  height: 1px; margin: 0 -12px;
}}
.cal[data-date-pane="calendar"] .dp-year-divider,
.cal[data-date-display="input"] .dp-year-divider,
[data-datepicker-docked][data-docked-pane="calendar"] .dp-year-divider {{
  display: none;
}}
...
[data-docked-trailing] {{
  width: 48px; height: 48px; flex-shrink: 0; border: none; background: transparent;
  display: flex; align-items: center; justify-content: center; cursor: pointer;
  font-size: 20px; line-height: 24px; color: inherit; padding: 0;
}}
.dp-years {{
...
.dp-range-input {{ display: flex; flex-direction: column; gap: 8px; }}
.dp-range-error {{
  font-size: 12px; line-height: 16px; padding: 0 12px 8px;
}}
.cal[data-range-input-errors] .dp-range-error-card,
.cal[data-date-input-errors] .dp-input-error-card {{ margin-bottom: 12px; }}
.dp-input-error {{
  font-size: 12px; line-height: 16px; padding: 0 12px 8px;
}}
...
document.querySelectorAll("[data-timepicker]").forEach(function (picker) {{
  function setDial(face) {{
    ...
    picker.querySelectorAll("[data-time-field]").forEach(function (el) {{
      var on = el.getAttribute("data-time-field") === face;
      el.setAttribute("data-active", on ? "1" : "0");
      var box = el.closest("[data-time-selector-colors]") || picker;
      el.style.background = on
        ? (box.getAttribute("data-time-selector-sel-bg") || el.style.background)
        : (box.getAttribute("data-time-selector-idle-bg") || el.style.background);
      el.style.color = on
        ? (box.getAttribute("data-time-selector-sel-fg") || el.style.color)
        : (box.getAttribute("data-time-selector-idle-fg") || el.style.color);
    }});
  }}
  picker.querySelectorAll("[data-hour]").forEach(function (el) {{
...
document.querySelectorAll("[data-scroll-display-mode-toggle]").forEach(function (btn) {{
  btn.addEventListener("click", function (ev) {{
    ...
    btn.textContent = mode === "scroll" ? "{time_kb}" : "{time_swipe}";
...
document.querySelectorAll("[data-date-year]").forEach(function (cell) {{
  cell.addEventListener("click", function (ev) {{
    ...
    paintDateYearToggle(host.querySelector("[data-date-year-toggle]"), "calendar");
  }});
}});
document.querySelectorAll("[data-date-range-live]").forEach(function (host) {{
  ...
  function dateOrd(y, m, d) {{ return y * 400 + m * 32 + d; }}
  function headerDateShort(y, m, d) {{
    return ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][m-1] + " " + d;
  }}
  function readEnd() {{
    var ey = host.getAttribute("data-range-end-year");
    if (!ey) return null;
    return {{
      y: parseInt(ey, 10),
      m: parseInt(host.getAttribute("data-range-end-month") || "0", 10),
      d: parseInt(host.getAttribute("data-range-end-day") || "0", 10)
    }};
  }}
  function readStart() {{
    var sy = host.getAttribute("data-range-start-year");
    if (!sy) return null;
    return {{
      y: parseInt(sy, 10),
      m: parseInt(host.getAttribute("data-range-start-month") || "0", 10),
      d: parseInt(host.getAttribute("data-range-start-day") || "0", 10)
    }};
  }}
  function writeSel(start, end) {{
    if (start) {{
      host.setAttribute("data-range-start-year", String(start.y));
      host.setAttribute("data-range-start-month", String(start.m));
      host.setAttribute("data-range-start-day", String(start.d));
    }} else {{
      host.removeAttribute("data-range-start-year");
      host.removeAttribute("data-range-start-month");
      host.removeAttribute("data-range-start-day");
    }}
    if (end) {{
      host.setAttribute("data-range-end-year", String(end.y));
      host.setAttribute("data-range-end-month", String(end.m));
      host.setAttribute("data-range-end-day", String(end.d));
    }} else {{
      host.removeAttribute("data-range-end-year");
      host.removeAttribute("data-range-end-month");
      host.removeAttribute("data-range-end-day");
    }}
    var headline = host.querySelector("[data-range-headline]");
    if (headline) {{
      if (start && end) headline.textContent = headerDateShort(start.y, start.m, start.d) + " – " + headerDateShort(end.y, end.m, end.d);
      else if (start) headline.textContent = headerDateShort(start.y, start.m, start.d) + " – End date";
      else headline.textContent = "Start date – End date";
    }}
    paintRangeFields(start, end);
  }}
  function fieldValue(d) {{
    if (!d) return "";
    return String(d.m).padStart(2, "0") + "/" + String(d.d).padStart(2, "0") + "/" + d.y;
  }}
  function paintRangeFields(start, end) {{
    var sv = host.querySelector("[data-range-live-start] .val");
    var ev = host.querySelector("[data-range-live-end] .val");
    if (sv) sv.textContent = fieldValue(start);
    if (ev) ev.textContent = fieldValue(end);
  }}
  function classify(y, m, day, kind) {{
    if (kind !== "InMonth") return kind;
    var start = readStart();
    var end = readEnd();
    var ty = parseInt(host.getAttribute("data-today-year") || "2026", 10);
  ...
  function paintMonthGrid(grid, y, m) {{
    ...
    var rngFg = host.getAttribute("data-day-range-fg") || "#1D192B";
    var todayBd = host.getAttribute("data-day-today") || "#6750A4";
    var inFg = host.getAttribute("data-day-in") || "#1C1B1F";
    var outFg = host.getAttribute("data-day-out") || "#9a9a9a";
    var html = "";
    monthGrid(y, m).forEach(function (cell) {{
      ...
      var dayA11y = kind === "Today" ? "Today" : (kind === "InRange" ? "In range" : "");
      var dayA11yAttr = dayA11y ? ' data-day-cell-a11y="1" title="'+dayA11y+'" aria-label="'+dayA11y+'"' : "";
      html += '<div class="day" data-day="'+day+'" data-kind="'+kind+'"'+dayA11yAttr+' data-range-fill="'+fill+'" style="background:'+cellBg+';color:'+fg+';border:'+outline+';border-radius:'+radius+'">'+inner+'</div>';
    }});
    grid.innerHTML = html;
  }}
  ...
  host.addEventListener("click", function (ev) {{
    ...
    var start = readStart();
    var end = readEnd();
    var tapped = {{ y: y, m: m, d: day }};
    if (!start) {{ start = tapped; end = null; }}
    else if (!end) {{
      if (dateOrd(tapped.y, tapped.m, tapped.d) >= dateOrd(start.y, start.m, start.d)) end = tapped;
      else {{ start = tapped; end = null; }}
    }} else {{ start = tapped; end = null; }}
    writeSel(start, end);
    paintRangeGrid();
  }});
  function addMonths(y, m, delta) {{
    ...
    var nm = ((idx % 12) + 12) % 12 + 1;
    if (ny < 1900) return [1900, 1];
  ...
  function paintMonthLabel() {{
    ...
    var y = parseInt(host.getAttribute("data-range-year") || "2026", 10);
    var m = parseInt(host.getAttribute("data-range-month") || "9", 10);
    label.textContent = monthNavLabel(y, m);
  }}
  host.querySelectorAll("[data-range-month-delta]").forEach(function (btn) {{
    btn.addEventListener("click", function (ev) {{
      ...
      var y = parseInt(host.getAttribute("data-range-year") || "2026", 10);
      var m = parseInt(host.getAttribute("data-range-month") || "9", 10);
      var next = addMonths(y, m, delta);
      host.setAttribute("data-range-year", String(next[0]));
      host.setAttribute("data-range-month", String(next[1]));
      paintMonthLabel();
      paintRangeGrid();
    }});
  }});
  ...
  if (yearToggle) {{
    ...
    yearToggle.addEventListener("click", function (ev) {{
      ev.stopPropagation();
      if (host.getAttribute("data-date-display") === "input") return;
      var pane = host.getAttribute("data-date-pane") === "year" ? "calendar" : "year";
      host.setAttribute("data-date-pane", pane);
      paintDateYearToggle(yearToggle, pane);
  ...
  host.addEventListener("click", function (ev) {{
    ...
    host.setAttribute("data-date-pane", "calendar");
    paintDateYearToggle(yearToggle, "calendar");
    paintMonthLabel();
    paintRangeGrid();
  }});
  var displayToggle = host.querySelector("[data-range-display-toggle]");
  if (displayToggle) {{
    displayToggle.addEventListener("click", function (ev) {{
      ev.stopPropagation();
  ...
  function readCommit(prefix) {{
    var y = host.getAttribute("data-range-commit-" + prefix + "-year");
    if (!y) return null;
    return {{
      y: parseInt(y, 10),
      m: parseInt(host.getAttribute("data-range-commit-" + prefix + "-month") || "0", 10),
      d: parseInt(host.getAttribute("data-range-commit-" + prefix + "-day") || "0", 10)
    }};
  }}
  function writeCommit(start, end) {{
    function set(prefix, d) {{
      if (d) {{
        host.setAttribute("data-range-commit-" + prefix + "-year", String(d.y));
        host.setAttribute("data-range-commit-" + prefix + "-month", String(d.m));
        host.setAttribute("data-range-commit-" + prefix + "-day", String(d.d));
      }} else {{
        host.removeAttribute("data-range-commit-" + prefix + "-year");
        host.removeAttribute("data-range-commit-" + prefix + "-month");
        host.removeAttribute("data-range-commit-" + prefix + "-day");
      }}
    }}
    set("start", start);
    set("end", end);
  ...
  function dismissRange() {{
    var start = readCommit("start");
    var end = readCommit("end");
    writeSel(start, end);
    if (start) {{
      host.setAttribute("data-range-year", String(start.y));
      host.setAttribute("data-range-month", String(start.m));
      paintMonthLabel();
    }}
    host.setAttribute("data-date-pane", "calendar");
    paintRangeGrid();
  }}
  var cancelBtn = host.querySelector("[data-range-cancel]");
  if (cancelBtn) {{
  ...
  var closeBtn = host.querySelector("[data-range-header-close]");
  if (closeBtn) {{
    closeBtn.addEventListener("click", function (ev) {{
      ev.stopPropagation();
      dismissRange();
  ...
  var okBtn = host.querySelector("[data-range-ok]");
  if (okBtn) {{
    okBtn.addEventListener("click", function (ev) {{
      ev.stopPropagation();
      writeCommit(readStart(), readEnd());
    }});
  }}
}});
document.querySelectorAll("[data-date-actions-live]").forEach(function (host) {{
  var months = ["January","February","March","April","May","June","July","August","September","October","November","December"];
  ...
  function readSelected() {{
    return {{
      y: parseInt(host.getAttribute("data-selected-year") || "2026", 10),
      m: parseInt(host.getAttribute("data-selected-month") || "9", 10),
      d: parseInt(host.getAttribute("data-selected-day") || "15", 10)
    }};
  }}
  function readCommit() {{
    return {{
      y: parseInt(host.getAttribute("data-date-commit-year") || "2026", 10),
      m: parseInt(host.getAttribute("data-date-commit-month") || "9", 10),
      d: parseInt(host.getAttribute("data-date-commit-day") || "15", 10)
    }};
  }}
  function writeSelected(d) {{
    host.setAttribute("data-selected-year", String(d.y));
    host.setAttribute("data-selected-month", String(d.m));
    host.setAttribute("data-selected-day", String(d.d));
  }}
  function writeCommit(d) {{
    host.setAttribute("data-date-commit-year", String(d.y));
    host.setAttribute("data-date-commit-month", String(d.m));
    host.setAttribute("data-date-commit-day", String(d.d));
  }}
  function headerDateLabel(d) {{
    return weekdays[weekdaySunday0(d.y, d.m, d.d)] + ", " + monthsShort[d.m - 1] + " " + d.d;
  }}
  function fieldValue(d) {{
    return String(d.m).padStart(2, "0") + "/" + String(d.d).padStart(2, "0") + "/" + d.y;
  }}
  function paintHeadline(d) {{
    var headline = host.querySelector("[data-date-headline]");
    if (headline) headline.textContent = headerDateLabel(d);
    var val = host.querySelector("[data-date-input-field] .val");
    if (val) val.textContent = fieldValue(d);
  }}
  function paintMonthLabel() {{
    var label = host.querySelector("[data-date-month]");
    if (!label) return;
    var y = parseInt(host.getAttribute("data-year") || "2026", 10);
    var m = parseInt(host.getAttribute("data-month") || "9", 10);
  ...
  host.querySelectorAll("[data-date-month-delta]").forEach(function (btn) {{
    btn.addEventListener("click", function (ev) {{
      ev.stopPropagation();
      if (host.getAttribute("data-date-pane") === "year") return;
      if (host.getAttribute("data-date-display") === "input") return;
      var delta = parseInt(btn.getAttribute("data-date-month-delta") || "0", 10);
      var y = parseInt(host.getAttribute("data-year") || "2026", 10);
      var m = parseInt(host.getAttribute("data-month") || "9", 10);
      var next = addMonths(y, m, delta);
      host.setAttribute("data-year", String(next[0]));
      host.setAttribute("data-month", String(next[1]));
      paintMonthLabel();
      paintDateGrid();
    }});
  }});
  ...
  function paintDateGrid() {{
    var grid = host.querySelector("[data-date-grid]");
    if (!grid) return;
    var y = parseInt(host.getAttribute("data-year") || "2026", 10);
    var m = parseInt(host.getAttribute("data-month") || "9", 10);
    var selBg = host.getAttribute("data-day-sel-bg") || "#6750A4";
    ...
    monthGrid(y, m).forEach(function (cell) {{
      ...
      var dayA11y = kind === "Today" ? "Today" : (kind === "InRange" ? "In range" : "");
      var dayA11yAttr = dayA11y ? ' data-day-cell-a11y="1" title="'+dayA11y+'" aria-label="'+dayA11y+'"' : "";
      html += '<div class="day" data-day="'+day+'" data-kind="'+kind+'"'+dayA11yAttr+' style="background:'+bg+';color:'+fg+';border:'+outline+';border-radius:'+radius+'">'+day+'</div>';
    }});
    grid.innerHTML = html;
  }}
  host.addEventListener("click", function (ev) {{
    ...
    var y = parseInt(host.getAttribute("data-year") || "2026", 10);
    var m = parseInt(host.getAttribute("data-month") || "9", 10);
    writeSelected({{ y: y, m: m, d: day }});
    paintHeadline({{ y: y, m: m, d: day }});
    paintDateGrid();
  }});
  host.querySelectorAll("[data-date-year]").forEach(function (cell) {{
    cell.addEventListener("click", function () {{
      paintMonthLabel();
      paintDateGrid();
    }});
  }});
  var cancelBtn = host.querySelector("[data-date-cancel]");
  if (cancelBtn) {{
    cancelBtn.addEventListener("click", function (ev) {{
      ev.stopPropagation();
      var committed = readCommit();
      writeSelected(committed);
      host.setAttribute("data-year", String(committed.y));
      host.setAttribute("data-month", String(committed.m));
      host.setAttribute("data-date-pane", "calendar");
      paintMonthLabel();
      paintHeadline(committed);
      paintDateGrid();
    }});
  }}
  var okBtn = host.querySelector("[data-date-ok]");
  if (okBtn) {{
    okBtn.addEventListener("click", function (ev) {{
      ev.stopPropagation();
      writeCommit(readSelected());
    }});
  }}
}});
...
document.querySelectorAll("[data-datepicker-docked]").forEach(function (dock) {{
  ...
  var trail = dock.querySelector("[data-docked-trailing]");
  if (trail) {{
  ...
  var monthsShort = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
  function dockedFieldValue(y, m, d) {{
    return monthsShort[m - 1] + " " + d + ", " + y;
  }}
  function writeDockedSelection(y, m, d) {{
    dock.setAttribute("data-selected-year", String(y));
    dock.setAttribute("data-selected-month", String(m));
    dock.setAttribute("data-selected-day", String(d));
    dock.setAttribute("data-year", String(y));
    dock.setAttribute("data-month", String(m));
    dock.setAttribute("data-docked-pane", "calendar");
    var val = dock.querySelector("[data-field-hero='docked-date'] .val");
    if (val) val.textContent = dockedFieldValue(y, m, d);
    var label = dock.querySelector("[data-docked-month-label]");
    var months = ["January","February","March","April","May","June","July","August","September","October","November","December"];
    if (label) label.textContent = months[m - 1] + " " + y + " ▾";
    paintDockedGrid(y, m);
  }}
  if (cal) cal.addEventListener("click", function (ev) {{
    ev.stopPropagation();
    if (dock.getAttribute("data-docked-select-live") === "1") {{
      ...
      if (dayEl && cal.contains(dayEl)) {{
        ...
        if (kind !== "OutOfMonth" && kind !== "Disabled") {{
          ...
          if (d) {{
            writeDockedSelection(y, m, d);
            if (dock.getAttribute("data-docked-dismiss-select") === "1") setOpen(false);
          }}
        }}
        return;
      }}
    }}
    var yearCell = ev.target.closest("[data-docked-year]");
    if (!yearCell || !cal.contains(yearCell)) return;
    var picked = parseInt(yearCell.getAttribute("data-docked-year") || "0", 10);
    if (!picked) return;
    ...
    dock.setAttribute("data-docked-pane", "calendar");
    paintDateYearToggle(dock.querySelector("[data-docked-year-toggle]"), "calendar");
    var yearLabel = dock.querySelector("[data-docked-month-label]");
    var monthNames = ["January","February","March","April","May","June","July","August","September","October","November","December"];
    if (yearLabel) yearLabel.textContent = monthNames[month - 1] + " " + year + " ▾";
...
"##,
        ...
        time_kb = time_picker::KEYBOARD_ICON,
        time_swipe = time_picker::SWIPE_VERTICAL_ICON,
...
fn paint_date_grid_fills(
    ...
    for ((day, kind), fill) in cells.into_iter().zip(fills) {
        ...
        let a11y_attr = match date_picker::day_cell_a11y(kind) {
            Some(label) => {
                format!(r#" data-day-cell-a11y="1" title="{label}" aria-label="{label}""#)
            }
            None => String::new(),
        };
        grid.push_str(&format!(
            "<div class=\"day\" data-day=\"{day}\" data-kind=\"{kind:?}\"{a11y_attr} data-range-fill=\"{fill}\" style=\"background:{cell_bg};color:{fg};border:{outline};border-radius:{radius}\">{inner}</div>",
            fill = fill.label()
        ));
    }
    grid
...
fn date_pickers(theme: &Theme) -> String {
    ...
    for d in date_picker::WEEKDAYS {
        week.push_str(&format!(
            "<div class=\"day\" data-date-weekday=\"1\" style=\"color:{};font-size:{}px\">{}</div>",
            a.weekday.css_hex(),
            a.weekday_style.size_sp,
            d
        ));
    }
    let grid = paint_date_grid(&a, cells);
    let empty_grid = paint_date_grid(&a, date_picker::month_grid_unselected(2026, 9, today));
    ...
    let range_empty_months = paint_range_months(
        &a,
        date_picker::RANGE_DEMO_START.year,
        date_picker::RANGE_DEMO_START.month,
...
<div class="cal dialog" data-date-container="1" data-datepicker-range="1" data-hero="datepicker-range" data-date-range-live="1" data-range-display-live="1" data-range-connector="1" data-range-vertical-months="1" data-date-header-paddings="1" data-date-range-header-paddings="1" data-range-header-chrome="1" data-date-display="picker" data-date-display-mode="picker" data-date-pane="calendar" data-week-start="sunday" data-range-year="2026" data-range-month="9" data-range-start-year="2026" data-range-start-month="9" data-range-start-day="15" data-range-end-year="2026" data-range-end-month="9" data-range-end-day="21" data-range-commit-start-year="2026" data-range-commit-start-month="9" data-range-commit-start-day="15" data-range-commit-end-year="2026" data-range-commit-end-month="9" data-range-commit-end-day="21" data-today-year="2026" data-today-month="9" data-today-day="11" data-day-sel-bg="{selbg}" data-day-sel-fg="{selfg}" data-day-range-bg="{rngbg}" data-day-range-fg="{rngfg}" data-day-today="{todaybd}" data-day-in="{infg}" data-day-out="{outfg}" data-year-sel-bg="{selbg}" data-year-sel-fg="{selfg}" data-year-idle-fg="{hy}" data-year-today-bd="{todaybd}" style="background:{bg};border-radius:{r}px;box-shadow:{sh};margin-bottom:16px">
  ...
  <div class="month-nav" data-range-month-nav="1" data-date-month-year="1" data-month-nav-a11y="1">
    <div data-range-month-label="1" data-range-year-toggle="1" title="{switch_to_year}" aria-label="{switch_to_year}">{range_month}</div>
    <div class="month-nav-arrows">
      <button type="button" data-range-month-delta="-1" data-month-nav-a11y="1" title="{range_prev}" aria-label="{range_prev}">&lt;</button>
      <button type="button" data-range-month-delta="1" data-month-nav-a11y="1" title="{range_next}" aria-label="{range_next}">&gt;</button>
    </div>
  </div>
  ...
  <div class="dp-divider" data-range-divider="1" style="background:{divc};height:{divh}px"></div>
  <div class="actions" data-range-actions="1" data-date-dialog-buttons="1">
    <button type="button" class="btn" data-range-cancel="1" style="background:transparent;color:{act}">{cancel}</button>
    <button type="button" class="btn" data-range-ok="1" style="background:transparent;color:{act}">{ok}</button>
  </div>
</div>
<h3>range picker empty</h3>
<p class="note">Compose <code>DateRangePickerTitle</code> / <code>DateRangePickerHeadline</code> when Picker has no selection: <code>Select dates</code> + <code>Start date – End date</code>, VerticalMonthsList with today outline and no selected or in-range days.</p>
<div class="cal dialog" data-date-container="1" data-datepicker-range-picker-empty="1" data-hero="datepicker-range-picker-empty" data-range-picker-empty="1" data-range-headline-empty="1" data-date-range-header-paddings="1" data-range-header-chrome="1" data-date-display="picker" data-date-display-mode="picker" data-range-vertical-months="1" data-week-start="sunday" data-selectable-dates="1" style="background:{bg};border-radius:{r}px;box-shadow:{sh};margin-bottom:16px">
  <div class="head" data-range-header-chrome="1" data-date-range-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <button type="button" data-range-header-close="1" aria-label="{range_close_label}">{range_close}</button>
    <div>
  ...
  <div class="week" data-date-weekdays="1">{week}</div>
  {range_empty_months}
  <div class="actions" data-date-dialog-buttons="1">
...
<div class="cal dialog" data-date-container="1" data-datepicker="1" data-hero="datepicker" data-date-actions-live="1" data-selectable-dates="1" data-week-start="sunday" data-date-header-paddings="1" data-date-display="picker" data-date-display-mode="picker" data-date-display-live="1" data-date-pane="calendar" data-year="2026" data-month="9" data-selected-year="2026" data-selected-month="9" data-selected-day="15" data-date-commit-year="2026" data-date-commit-month="9" data-date-commit-day="15" data-today-year="2026" data-today-month="9" data-today-day="11" data-day-sel-bg="{selbg}" data-day-sel-fg="{selfg}" data-day-today="{todaybd}" data-day-in="{infg}" data-day-out="{outfg}" data-year-sel-bg="{selbg}" data-year-sel-fg="{selfg}" data-year-idle-fg="{hy}" data-year-today-bd="{todaybd}" style="background:{bg};border-radius:{r}px;box-shadow:{sh};margin-bottom:16px">
  <div class="head" data-date-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <div>
      <div data-date-title-pad="1" style="color:{hy};font-size:{ys}px">Select date</div>
  ...
  <div class="dp-entry-divider" data-date-entry-divider="1" style="background:{divc};height:{divh}px"></div>
  <div class="month-nav" data-date-month-nav="1" data-date-month-year="1" data-month-nav-a11y="1">
    <div data-date-month="1" data-date-year-toggle="1" title="{switch_to_year}" aria-label="{switch_to_year}">{month}</div>
    <div class="month-nav-arrows">
      <button type="button" data-date-month-delta="-1" data-month-nav-a11y="1" title="{date_prev}" aria-label="{date_prev}">&lt;</button>
      <button type="button" data-date-month-delta="1" data-month-nav-a11y="1" title="{date_next}" aria-label="{date_next}">&gt;</button>
    </div>
  </div>
  <div class="week" data-date-weekdays="1">{week}</div>
  ...
  <div class="actions" data-date-actions="1" data-date-dialog-buttons="1">
    <button type="button" class="btn" data-date-cancel="1" style="background:transparent;color:{act}">{cancel}</button>
    <button type="button" class="btn" data-date-ok="1" style="background:transparent;color:{act}">{ok}</button>
  </div>
</div>
<h3>modal picker empty</h3>
<p class="note">Compose <code>DatePickerHeadline</code> when Picker has no selection: headlineLarge <code>Selected date</code> (<code>m3c_date_picker_headline</code>), calendar with today outline and no selected day.</p>
<div class="cal dialog" data-date-container="1" data-datepicker-picker-empty="1" data-hero="datepicker-picker-empty" data-picker-empty="1" data-date-headline-empty="1" data-date-display="picker" data-date-display-mode="picker" data-selectable-dates="1" data-week-start="sunday" style="background:{bg};border-radius:{r}px;box-shadow:{sh};margin-bottom:16px">
  <div class="head" data-date-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <div>
  ...
  <div class="dp-entry-divider" data-date-entry-divider="1" style="background:{divc};height:{divh}px"></div>
  <div class="month-nav" data-date-month-year="1" data-month-nav-a11y="1">
    <div title="{switch_to_year}" aria-label="{switch_to_year}">{month}</div>
    <div class="month-nav-arrows">
      <button type="button" data-month-nav-a11y="1" title="{date_prev}" aria-label="{date_prev}">&lt;</button>
      <button type="button" data-month-nav-a11y="1" title="{date_next}" aria-label="{date_next}">&gt;</button>
    </div>
  </div>
  <div class="week" data-date-weekdays="1">{week}</div>
  <div class="grid" data-picker-empty-grid="1">{empty_grid}</div>
  <div class="actions" data-date-dialog-buttons="1">
    <button class="btn" style="background:transparent;color:{act}">{cancel}</button>
    <button class="btn" style="background:transparent;color:{act}">{ok}</button>
  </div>
</div>
<div class="docked" data-datepicker-docked="1" data-hero="datepicker-docked" data-popup="open" data-dismiss-outside="1" data-docked-select-live="{docked_live}" data-docked-dismiss-select="{docked_dismiss}" data-docked-trailing="{docked_trail}" data-docked-pane="calendar" data-year="2026" data-month="9" data-selected-year="2026" data-selected-month="9" data-selected-day="15" data-today-year="2026" data-today-month="9" data-today-day="11" data-day-sel-bg="{selbg}" data-day-sel-fg="{selfg}" data-day-today="{todaybd}" data-day-in="{infg}" data-day-out="{outfg}" data-year-sel-bg="{selbg}" data-year-sel-fg="{selfg}" data-year-idle-fg="{hy}" data-year-today-bd="{todaybd}">
  {docked_field}
  <div class="cal dialog" data-date-container="1" data-datepicker-popup="open" style="background:{bg};border-radius:8px {r}px {r}px {r}px;box-shadow:{sh};margin-top:4px;width:100%">
    <div class="month-nav" data-date-month-year="1" data-month-nav-a11y="1">
      <div data-docked-month-label="1" data-docked-year-toggle="1" title="{switch_to_year}" aria-label="{switch_to_year}">{month}</div>
      <div class="month-nav-arrows">
        <button type="button" data-docked-month="-1" data-month-nav-a11y="1" title="{date_prev}" aria-label="{date_prev}">&lt;</button>
        <button type="button" data-docked-month="1" data-month-nav-a11y="1" title="{date_next}" aria-label="{date_next}">&gt;</button>
      </div>
    </div>
    <div class="week" data-date-weekdays="1">{week}</div>
...
<h3>modal input</h3>
<p class="note">Compose <code>DatePickerDisplayMode.Input</code>: headline + supporting text, outlined <code>MM/DD/YYYY</code> field, calendar/edit toggle, Cancel / OK. Supporting-text errors use Compose <code>DateInputValidator</code> (<code>Date format not recognized</code> / year-range).</p>
<div class="cal dialog" data-date-container="1" data-datepicker-input="1" data-hero="datepicker-input" data-date-display="input" data-date-display-mode="input">
  <div class="head" data-date-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <div>
      <div data-date-title-pad="1" style="color:{hy};font-size:{ys}px">{input_headline}</div>
...
<h3>modal input empty</h3>
<p class="note">Compose <code>DatePickerHeadline</code> when Input has no selection: headlineLarge <code>Entered date</code> (<code>m3c_date_input_headline</code>), empty outlined <code>MM/DD/YYYY</code>.</p>
<div class="cal dialog" data-date-container="1" data-datepicker-input-empty="1" data-hero="datepicker-input-empty" data-date-input-empty="1" data-date-headline-empty="1" data-date-display="input" data-date-display-mode="input">
  <div class="head" data-date-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <div>
      <div data-date-title-pad="1" style="color:{hy};font-size:{ys}px">{input_headline}</div>
...
<div class="cal dialog" data-date-container="1" data-datepicker-input-errors="1" data-hero="datepicker-input-errors" data-date-input-errors="1" data-date-display="input">
  <div class="dp-input-error-card" data-date-error="format">
    <div class="dp-input">{input_format_field}</div>
    <div class="dp-input-error" data-date-error-label="format" style="color:{errc}">{format_err}</div>
  </div>
...
<div class="cal dialog" data-date-container="1" data-datepicker-range-input="1" data-hero="datepicker-range-input" data-date-range-header-paddings="1" data-range-header-chrome="1" data-date-display="input" data-date-display-mode="input">
  <div class="head" data-range-header-chrome="1" data-date-range-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <button type="button" data-range-header-close="1" aria-label="{range_close_label}">{range_close}</button>
    <div>
...
<h3>range input empty</h3>
<p class="note">Compose <code>DateRangePickerHeadline</code> when both ends are unset: headlineLarge <code>Start date – End date</code>, empty outlined Start/End <code>MM/DD/YYYY</code>.</p>
<div class="cal dialog" data-date-container="1" data-datepicker-range-input-empty="1" data-hero="datepicker-range-input-empty" data-range-input-empty="1" data-range-headline-empty="1" data-date-range-header-paddings="1" data-range-header-chrome="1" data-date-display="input" data-date-display-mode="input">
  <div class="head" data-range-header-chrome="1" data-date-range-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <button type="button" data-range-header-close="1" aria-label="{range_close_label}">{range_close}</button>
    <div>
      <div data-date-range-title-pad="1" style="color:{hy};font-size:{ys}px">{range_input_headline}</div>
...
<div class="cal dialog" data-date-container="1" data-datepicker-range-start-only="1" data-hero="datepicker-range-start-only" data-range-start-only="1" data-range-headline-start-only="1" data-date-range-header-paddings="1" data-range-header-chrome="1" data-date-display="input" data-date-display-mode="input">
  <div class="head" data-range-header-chrome="1" data-date-range-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <button type="button" data-range-header-close="1" aria-label="{range_close_label}">{range_close}</button>
    <div>
      <div data-date-range-title-pad="1" style="color:{hy};font-size:{ys}px">{range_input_headline}</div>
...
<h3>range input end only</h3>
<p class="note">Compose <code>DateRangePickerHeadline</code> when only end is set: headlineLarge <code>Start date – Sep 21</code> (<code>mtrl_picker_range_header_only_end_selected</code>), Start still the <code>MM/DD/YYYY</code> pattern, End outlined <code>09/21/2026</code>.</p>
<div class="cal dialog" data-date-container="1" data-datepicker-range-end-only="1" data-hero="datepicker-range-end-only" data-range-end-only="1" data-range-headline-end-only="1" data-date-range-header-paddings="1" data-range-header-chrome="1" data-date-display="input" data-date-display-mode="input">
  <div class="head" data-range-header-chrome="1" data-date-range-header-min="1" style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px">
    <button type="button" data-range-header-close="1" aria-label="{range_close_label}">{range_close}</button>
    <div>
      <div data-date-range-title-pad="1" style="color:{hy};font-size:{ys}px">{range_input_headline}</div>
...
<div class="cal dialog" data-date-container="1" data-datepicker-range-input-errors="1" data-hero="datepicker-range-input-errors" data-range-input-errors="1" data-date-display="input">
...
<h3>year picker</h3>
<p class="note">Compose <code>YearPicker</code>: 3-column 72×36 pills, <code>YearRange</code> 1900–2100, selected primary, current-year outline. Month ▾ opens this pane. Official <code>SwitchableDateEntryContent</code> paints a <code>HorizontalDivider</code> under the year grid.</p>
<div class="cal dialog" data-date-container="1" data-datepicker-year="1" data-hero="datepicker-year" data-date-pane="year" data-date-display="picker" data-year="2026" data-year-sel-bg="{selbg}" data-year-sel-fg="{selfg}" data-year-idle-fg="{hy}" data-year-today-bd="{todaybd}" style="background:{bg};border-radius:{r}px;box-shadow:{sh};margin-bottom:16px">
  <div class="head" data-date-header-min="1">
    <div data-date-title-pad="1" style="color:{hy};font-size:{ys}px">Select date</div>
    <div data-date-headline-pad="1" style="color:{hd};font-size:{ds}px;font-weight:{dw}">{headline}</div>
  </div>
  <div class="dp-entry-divider" data-date-entry-divider="1" style="background:{divc};height:{divh}px"></div>
  ...
  <div class="dp-year-divider" data-date-year-divider="1" style="background:{divc};height:{ydh}px"></div>
        ...
        range_close = date_picker::RANGE_HEADER_CLOSE_GLYPH,
        range_close_label = date_picker::RANGE_HEADER_CLOSE_LABEL,
        ...
        range_prev = date_picker::RANGE_PREV_MONTH,
        range_next = date_picker::RANGE_NEXT_MONTH,
        date_prev = date_picker::DATE_PREV_MONTH,
        ...
        docked_dismiss = if date_picker::DOCKED_DISMISS_ON_SELECT {
            "1"
        } else {
            "0"
        },
        docked_trail = if date_picker::DOCKED_TRAILING {
            "1"
        } else {
            "0"
        },
        divc = theme.color.outline_variant.css_hex(),
        divh = date_picker::DATE_ENTRY_DIVIDER_H_DP,
        ydh = date_picker::YEAR_PICKER_DIVIDER_H_DP,
        act = theme.color.primary.css_hex(),
        selbg = a.day_selected_container.css_hex(),
        selfg = a.day_selected.css_hex(),
        rngbg = a.day_range_container.css_hex(),
        rngfg = a.day_range.css_hex(),
        ...
        docked_field = paint_outlined_field(
            ...
            &format!(
                r#"<div class="docked-field-row"><div class="val">{}</div>{}</div>"#,
                date_picker::docked_field_value(selected),
                if date_picker::DOCKED_TRAILING {
                    format!(
                        r#"<button type="button" data-docked-trailing="1" aria-label="{}">{}</button>"#,
                        date_picker::DOCKED_TRAILING_LABEL,
        ...
        empty_headline = date_picker::INPUT_EMPTY_HEADLINE,
        picker_empty_headline = date_picker::PICKER_EMPTY_HEADLINE,
        input_supporting = date_picker::INPUT_SUPPORTING,
        range_input_headline = date_picker::RANGE_INPUT_HEADLINE,
        range_empty_headline = date_picker::RANGE_EMPTY_HEADLINE,
        range_picker_title = date_picker::RANGE_PICKER_TITLE,
        ...
        range_empty_start = paint_outlined_field(
            &text_field::resolve(
                theme,
                text_field::TextFieldVariant::Outlined,
                InteractionState::Enabled,
            ...
            r#"data-date-range-empty-start="1""#,
            date_picker::RANGE_START_LABEL,
            &format!(
        ...
        range_empty_end = paint_outlined_field(
            &text_field::resolve(
                theme,
                text_field::TextFieldVariant::Outlined,
                InteractionState::Enabled,
        ...
        range_end_only_start = paint_outlined_field(
            ...
            r#"data-date-range-end-only-start="1""#,
            date_picker::RANGE_START_LABEL,
            &format!(
                r#"<div class="val" data-date-placeholder="1">{}</div>"#,
                date_picker::INPUT_PLACEHOLDER
        ...
        range_end_only_end = paint_outlined_field(
            &text_field::resolve(
                theme,
                text_field::TextFieldVariant::Outlined,
                InteractionState::Enabled,
            ...
            r#"data-date-range-end-only-end="1""#,
            date_picker::RANGE_END_LABEL,
            &format!(
                r#"<div class="val">{}</div>"#,
        ...
        range_live_start = paint_outlined_field(
            &text_field::resolve(
                theme,
                text_field::TextFieldVariant::Outlined,
                InteractionState::Enabled,
        ...
        input_format_field = paint_outlined_field(
            ...
            r#"data-date-input-error="format" data-field-error="1""#,
        ...
        input_year_field = paint_outlined_field(
            &text_field::resolve(
                theme,
                text_field::TextFieldVariant::Outlined,
                InteractionState::Error,
            ...
            r#"data-date-input-error="year" data-field-error="1""#,
            date_picker::INPUT_FIELD_LABEL,
            &format!(
                r#"<div class="val">{}</div>"#,
        ...
        range_live_end = paint_outlined_field(
            &text_field::resolve(
                theme,
                text_field::TextFieldVariant::Outlined,
                InteractionState::Enabled,
        ...
        empty_input_field = paint_outlined_field(
            &text_field::resolve(
                theme,
                text_field::TextFieldVariant::Outlined,
                InteractionState::Enabled,
            ...
            r#"data-date-input-placeholder="1""#,
            date_picker::INPUT_FIELD_LABEL,
            &format!(
                r#"<div class="val" data-date-placeholder="1">{}</div>"#,
                date_picker::INPUT_PLACEHOLDER
...
fn paint_contained_search_filtered(
    ...
    let mut rows = String::new();
    let mut flat = 0usize;
    let flatten = !status.shows_suggestion_groups();
    let flat_count = result_count;
    for (title, items) in grouped {
        let group_len = items.len();
        rows.push_str(&format!(
            r#"<div class="sv-group" data-search-group="{title}" data-search-list="segmented"><div class="sv-group-title" style="color:{ico}">{title}</div>"#,
            title = title,
            ico = contained.suggestion_icon.css_hex(),
        ));
        for (gi, label) in items.into_iter().enumerate() {
            ...
            let lead_kind = search::row_leading_kind(status, label);
            let lead_sz = search::row_leading_size_dp(status, lead_kind);
            let lead_fg = search::row_leading_content(theme, lead_kind);
            let lead_bg = search::row_leading_container(theme, lead_kind);
            let lead_style = match lead_bg {
                Some(bg) => format!(
                    "width:{sz}px;height:{sz}px;background:{bg};color:{fg};border-radius:{r}px",
                    sz = lead_sz,
                    bg = bg.css_hex(),
                    fg = lead_fg.css_hex(),
                    r = lead_sz / 2.0,
                ...
                None => format!(
                    "width:{sz}px;height:{sz}px;color:{fg}",
                    sz = lead_sz,
                    fg = lead_fg.css_hex(),
                ),
            };
            rows.push_str(&format!(
                ...
                glyph = search::row_leading_glyph(lead_kind, label, flat),
                sel = selected as u8,
...
fn paint_scroll_field(
    ...
    format!(
        r#"<div class="scroll-field" data-scroll-field="{kind}" data-time-field-shape="large" data-count="{count}" data-offset="{off}" data-field-h="{h}" style="width:{w}px;height:{h}px;background:{bg};border-radius:{r}px">{items}</div>"#,
...
fn time_clock_html(
    ...
    for fmt in time_picker::TimeFormat::ALL {
        ...
        for cell in time_picker::hour_cells(fmt, clock_dp, number_dp) {
            ...
            hours.push_str(&format!(
                r#"<div class="hour" data-hour="{h}" data-clock-number-a11y="1" data-ring="{ring}" data-format="{fmt}" data-selected="{sel}" title="{a11y}" aria-label="{a11y}" style="display:{disp};left:{x}px;top:{y}px;width:{n}px;height:{n}px;background:{bg};color:{fg};font-weight:{fw}">{label}</div>"#,
                h = cell.hour,
                ...
                fw = fw,
                label = cell.label,
                a11y = time_picker::clock_number_label(
    ...
    for m in time_picker::minute_labels() {
        ...
        minutes.push_str(&format!(
            r#"<div class="minute" data-minute="{m}" data-clock-number-a11y="1" data-selected="{sel}" title="{a11y}" aria-label="{a11y}" style="left:{x}px;top:{y}px;width:{n}px;height:{n}px;background:{bg};color:{fg};font-weight:{fw}">{label}</div>"#,
            m = m,
            sel = selected as u8,
            x = x,
            y = y,
  ...
  <div class="time-input" data-time-input="1" data-time-picker-style="input" data-time-field-outline="1" data-hour="{hour}" data-minute="{minute}" data-period="{period}" data-time-field-bg="{imbg}" data-time-field-fg="{imfg}" data-time-field-bd="{imbd}" data-time-field-focus-bg="{ifbg}" data-time-field-focus-fg="{iffg}" data-time-field-focus-bd="{ifbd}">
    <div class="time-input-row">
      <div class="time-input-col">
        <input class="time-input-field" data-time-input-field="hour" data-hour-minute-a11y="1" data-time-field-shape="large" data-focused="1" maxlength="2" inputmode="numeric" value="{ihh}" title="{hour_field_a11y}" aria-label="{hour_field_a11y}" style="background:{ifbg};color:{iffg};width:{ifw}px;height:{ifh}px;border-radius:{ifr}px;border:{ifow} solid {ifbd}"/>
      ...
      <div class="time-input-col">
        <input class="time-input-field" data-time-input-field="minute" data-hour-minute-a11y="1" data-time-field-shape="large" maxlength="2" inputmode="numeric" value="{imm}" title="{minute_field_a11y}" aria-label="{minute_field_a11y}" style="background:{imbg};color:{imfg};width:{ifw}px;height:{ifh}px;border-radius:{ifr}px;border:{imow} solid {imbd}"/>
...
<h3>dial</h3>"#,
        ...
        hour_field_a11y = time_picker::INPUT_HOUR_FIELD,
        minute_field_a11y = time_picker::INPUT_MINUTE_FIELD,
        mode = mode.label(),
    ...
    let selectors = format!(
        r#"<div class="time-fields" data-time-selector-colors="1" data-time-selector-sel-bg="{tsb}" data-time-selector-sel-fg="{tsf}" data-time-selector-idle-bg="{tib}" data-time-selector-idle-fg="{tif}">
      <div class="time-field" data-time-field="hour" data-hour-minute-a11y="1" data-active="{ha}" title="{hour_sel}" aria-label="{hour_sel}" style="font-size:{ds}px;font-weight:{dw};color:{hfg};background:{hbg}">{hh}</div>
      <div class="time-sep" data-display-separator="1" style="font-size:{ds}px;font-weight:{dw};color:{hd}">:</div>
      <div class="time-field" data-time-field="minute" data-hour-minute-a11y="1" data-active="{ma}" title="{minute_sel}" aria-label="{minute_sel}" style="font-size:{ds}px;font-weight:{dw};color:{mfg};background:{mbg}">{mm}</div>
    </div>
    <div class="period" data-period-toggle-margin="1" data-period-outline="1" data-period-toggle-a11y="1" title="{period_toggle}" aria-label="{period_toggle}" style="border-color:{pob}">
      ...
      <button data-period="PM" style="background:{pmb};color:{pmf}">{pm}</button>
        ...
        tsb = a.time_selector_selected_container.css_hex(),
        tsf = a.time_selector_selected.css_hex(),
        tib = a.time_selector_container.css_hex(),
        tif = a.time_selector_content.css_hex(),
        hbg = if hour_active {
            a.time_selector_selected_container.css_hex()
        } else {
            a.time_selector_container.css_hex()
        },
        hfg = if hour_active {
            a.time_selector_selected.css_hex()
        } else {
            a.time_selector_content.css_hex()
        },
        mbg = if hour_active {
            a.time_selector_container.css_hex()
        } else {
