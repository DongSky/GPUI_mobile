# v25 leftovers — GPUI mobile Material 3 Expressive

Continuing from **v24** (`feat: v24 Expressive standard button groups, list swipe/reorder, tooltip hover`). Critique vs [m3.material.io](https://m3.material.io/components/button-groups/specs) Expressive.

**v24 landed on `main` only (no PR).** Start from that tip; do not re-open standard button groups (12dp gap, ExpandedRatio 0.15 neighbor morph, Start/Center/End), list swipe Archive/Delete rails, drag-handle reorder, or tooltip hover / 500ms long-press as missing inventory — they ship. Navigation drawer and **banner** stay **out** of inventory.
...
8. **Live GPUI + Chrome recapture** — this VM cannot create a wgpu surface; Chrome `--screenshot` hangs (dump-dom works). Official-vs-catalog strips remain v19 left halves. Recapture HTML heroes (including standard groups, list swipe/reorder, tooltip hover) when a display/screenshot pipeline works.
9. **List swipe is catalog/host-demo, not a nested-scroll LazyColumn** — GPUI uses scroll-wheel analog; HTML pointer-drag. True finger fling + predictive-back stay unimplemented. Icon-button narrow/wide widths are still size-only (XS–XL), not the Expressive width axis.
...
1. Licensed camera stills (or icon-button narrow/wide if photos stay deferred).
2. Live IME / caret / search transform (needs NDK + gpui API).
3. APK install + Chrome/wgpu recapture of v21–v24 heroes.
