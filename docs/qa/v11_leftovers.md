...
3. **Outlined notch** — GPUI and HTML now share the explicit C-path even-odd polygon. Remaining: lyon `LineCap` if hosts want a stroked centerline instead of a filled ring; label width vs glyph metrics can still leave a sliver of stroke under wide letters.
