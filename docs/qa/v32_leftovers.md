# v33 leftovers — GPUI mobile Material 3 Expressive

Continuing from **v32** (`feat: v32 Expressive overlay menu submenu + live typeahead`). Critique vs [m3.material.io](https://m3.material.io/components/menus/specs) Expressive.

**v32 landed on `main` only (no PR).** Start from that tip; do not re-open overlay submenu (grouped default, More Stay + End flyout, Inactive 8 / Active 24, live typeahead, catalog `menu-overlay`) as missing inventory — they ship. Navigation drawer and **banner** stay **out** of inventory.
...
7. **Live GPUI + Chrome recapture** — this VM cannot create a wgpu surface; Chrome `--screenshot` hangs (dump-dom works). Official-vs-catalog strips remain v19 left halves. Recapture HTML heroes (including Expressive overlay flyout) when a display/screenshot pipeline works.
...
11. **Menu overlay as unscrimmed popup** — GPUI overlay still uses a 32% scrim full-screen host (catalog/demo). Official dropdowns sit next to an anchor without a modal scrim. Overflow/split menus are still a single 16dp shell (no grouped 2dp gap + nested More flyout). Hover-open has no delay; GPUI typeahead needs the cascade focused.
...
1. ElevatedFilterChip / tonal chip color defaults (implementable) or unscrimmed anchored overlay / overflow grouping.
2. Live IME / caret / search transform (needs NDK + gpui API).
3. APK install + Chrome/wgpu recapture of v21–v32 heroes.
