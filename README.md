...
| Date picker | Date pickers | done | Modal calendar; SelectableDates greys out Sat/Sun on the Picker grid; DatePickerTitlePadding / DatePickerHeadlinePadding (start 24 / end 12 / top 16 / bottom 12); DatePickerModeTogglePadding (end 12 / bottom 12); DateRangePickerTitlePadding / DateRangePickerHeadlinePadding (start 64 / end 12 / bottom 12, no Save/X chrome); range hero with live start→end, month nav, YearPicker, showModeToggle, Confirm/Cancel, drawRangeBackground half-cell connectors, VerticalMonthsList + DateRangePickerTitle Picker empty (Select dates); modal Confirm/Cancel + month nav; docked popup + trailing DateRange icon + YearPicker + live day select; modal Input InputTextFieldPadding 24/24/10 + bottom 16 + DateInputValidator format/year-range/not-allowed (SelectableDates weekends) + DatePickerHeadline Picker empty (Selected date) + Input empty (Entered date); range Input DateInputValidator errors (format / year range / not-allowed / order) + DateRangePickerHeadline empty (Start date – End date) + start-only (Sep 15 – End date) + end-only (Start date – Sep 21) | [spec](https://m3.material.io/components/date-pickers/specs) |
