...
| Motion tokens | Easing and duration | done | Expressive spatial/effects springs + legacy emphasized eval; catalog CSS morph; shared FRAME_MS/FRAME_DT vsync clock for second-hand + TimeScroll fling + carousel + list swipe | [spec](https://m3.material.io/styles/motion/easing-and-duration/tokens-specs) |
...
| Button group | Standard + connected button groups (Expressive) | done | Standard: 12dp gap, ExpandedRatio 0.15 neighbor morph, Start/Center/End tonal→filled square + trailing filled OverflowIndicator (Left/Right/Justify + More › grouped 2dp flyout). Connected: 2dp gap, 8dp inner, full-round outer; Day/Week/Month + icon row with Cut/Copy/Paste + More overflow | [spec](https://m3.material.io/components/button-groups/specs) |
| Icon button | Icon buttons | done | Expressive XS–XL (32/40/56/96/136), narrow/default/wide (Compose IconButtonWidthOption; S 32/40/52), round/square, press morph; toggle selected colors + round↔square (Compose IconToggleButton); default S 40×24 | [spec](https://m3.material.io/components/icon-buttons/specs) |
...
| FAB menu | FAB menu (Expressive) | done | 2–6 item pills 56dp full-round + 56dp close FAB; 8dp close gap / 4dp item gap; primary/secondary/tertiary sets; Document / Message / Folder hero over licensed woven-basket still | [spec](https://m3.material.io/components/fab-menu/specs) |
| Split button | Split buttons (Expressive) | done | Leading action + trailing menu, 2dp gap; outer full-round, inner 4dp rest / 12dp press (S); open trailing goes full-round + pressed layer; trailing menu is grouped 2dp (Add to cart / Save for later + More ›); licensed enamel-mugs still product card ($7.49) | [spec](https://m3.material.io/components/split-button/specs) |
| Toolbar | Floating / docked toolbars (Expressive) | done | Floating 64dp full-round surface-container / vibrant primary-container + tertiary FAB; docked full-width; horizontal + vertical; 8dp pad / 4dp item gap; chat-thread scene (Renee Claess + licensed dog still) | [spec](https://m3.material.io/components/toolbars/specs) |
| Text field | Filled and outlined text fields | done | Expressive roundedShape CornerMedium 12 + tonalColors (filled SurfaceContainer / outlined OnPrimary, Inside label, no indicator / OutlineVariant); baseline extra-small + Cutout notch still available; outlined C-path even-odd + lyon centerline; IME JNI-mangled Java_dev_gpui_… exports + RegisterNatives fnPtr + session handle + CursorAnchorInfo | [spec](https://m3.material.io/components/text-fields/specs) |
| List | Expressive segmented + baseline lists | done | Expressive segmented (recommended): 2dp gap, 4dp inner / 16dp outer, selected 16dp + secondary-container; Wi-Fi switches; swipe Archive/Delete 80dp rails + LazyColumn fling (Closed/Open/primary, 16dp overshoot, growing reveal); drag-handle reorder. Baseline 56/72/88 0-corner still available | [spec](https://m3.material.io/components/lists/specs) |
...
| Chip | Assist / filter / input / suggestion chips | done | 32dp; Filter/Input Expressive ChipShapes morph (medium 12 / full selected / small 8 pressed) + selected check + live rememberAnimatedShape press interpolation + GPUI with_animation press clock; tonal leading icon on-surface-variant; ElevatedFilterChip surface-container-low elev 1; InputChip 24dp avatar + compact 4dp arrangement; Assist/Suggestion stay full-round | [spec](https://m3.material.io/components/chips/specs) |
...
| Snackbar | Snackbar | done | Inverse surface 48dp; Gmail scene with licensed camera-still avatars + Mail/Chat/Rooms/Meet filled dest icons + peeking thread + Email archived / Action / close; timeout 4s/10s + swipe 72dp | [spec](https://m3.material.io/components/snackbar/specs) |
...
| Navigation rail | Navigation rail | done | WideNavigationRailItem Top/Start: collapsed 96 (live narrow modal 80↔220) 56×32; expanded 220–360 Start 56dp pill; live Top→Start iconPosition lerp; in-flow standard 96↔220 no scrim CornerNone/Surface + Extended FAB Create (56↔188); modal overlay 96 + 32% scrim + CornerLarge 16 / SurfaceContainer; ContentPadding 0/44/0/44 (WNRVerticalPadding = TopSpace); hideOnCollapse slide + Arrangement.Center + expanded modal shape + always-extended Create (~188); header Menu/MenuOpen + Extended FAB Create (56↔188) + tooltip Above + Arrangement.Bottom; header-less modal/narrow Extended FAB Create (56↔188); active label secondary | [spec](https://m3.material.io/components/navigation-rail/specs) |
...
| Bottom sheet | Bottom sheets | done | surface-container-low, extra-large top 28dp, 32×4 handle, elev 1; share sheet over licensed album still with Send people row + horizontal Share/Add to/Trash actions | [spec](https://m3.material.io/components/bottom-sheets/specs) |
...
| Date picker | Date pickers | done | Modal calendar; SelectableDates greys out Sat/Sun on the Picker grid; DatePickerDialog ContainerWidth 360 / ContainerHeight 568; HeaderContainerHeight 120; DateEntryContainer header HorizontalDivider (under title/headline/toggle, outline-variant 1dp; docked omitted); RangeSelectionHeaderContainerHeight 128 − HeaderHeightOffset 60; DatePickerTitlePadding / DatePickerHeadlinePadding (start 24 / end 12 / top 16 / bottom 12); DatePickerModeTogglePadding (end 12 / bottom 12); DateRangePickerTitlePadding / DateRangePickerHeadlinePadding (start 64 / end 12 / bottom 12) + range-header chrome (leading close/back); DatePickerDialog DialogButtonsPadding (end 6 / bottom 8) + 8dp main-axis; MonthsNavigation MonthYearHeight 56 + 48dp arrows at end; range hero with live start→end, month nav, YearPicker, showModeToggle, Confirm/Cancel, drawRangeBackground half-cell connectors, VerticalMonthsList + DateRangePickerTitle Picker empty (Select dates); modal Confirm/Cancel + month nav; docked popup + trailing DateRange icon + YearPicker + live day select; modal Input InputTextFieldPadding 24/24/10 + bottom 16 + DateInputValidator format/year-range/not-allowed (SelectableDates weekends) + DatePickerHeadline Picker empty (Selected date) + Input empty (Entered date); range Input DateInputValidator errors (format / year range / not-allowed / order) + DateRangePickerHeadline empty (Start date – End date) + start-only (Sep 15 – End date) + end-only (Start date – Sep 21) | [spec](https://m3.material.io/components/date-pickers/specs) |
| Search | Search bar + view | done | Expressive contained: compact <600dp fullscreen (0/0); medium ≥600dp docked Corner 28 + 24→12dp; Recent / Suggestions 8dp groups; All/Apps/Shortcuts/Settings filter chips; divided activity still available | [spec](https://m3.material.io/components/search/specs) |
| Time picker | Time pickers (dial + TimeScroll) | done | Expressive TimeScroll + TimeInput + 24h ClockFace (outer 00–11 / inner 12–23, 101/69dp); ClockFace number a11y (o'clock / minutes / hours); ScrollFieldDefaults.shape / TimePickerDefaults.shapes() CornerLarge 16; ClockDisplayBottomMargin 36 + ClockFaceBottomMargin 24 on Vertical; DisplaySeparatorWidth 24 + PeriodToggleMargin 12 | [spec](https://m3.material.io/components/time-pickers/specs) |
