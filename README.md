...
| Date picker | Date pickers | done | Modal calendar; range hero with live start→end, month nav, YearPicker, showModeToggle, Confirm/Cancel, drawRangeBackground half-cell connectors, VerticalMonthsList; modal Confirm/Cancel + month nav; docked popup + trailing DateRange icon + YearPicker + live day select; modal Input DateInputValidator format/year-range; range Input DateInputValidator errors (format / year range / order) | [spec](https://m3.material.io/components/date-pickers/specs) |
