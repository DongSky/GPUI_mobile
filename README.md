...
| Date picker | Date pickers | done | Modal calendar; SelectableDates greys out Sat/Sun on the Picker grid; range hero with live start→end, month nav, YearPicker, showModeToggle, Confirm/Cancel, drawRangeBackground half-cell connectors, VerticalMonthsList; modal Confirm/Cancel + month nav; docked popup + trailing DateRange icon + YearPicker + live day select; modal Input DateInputValidator format/year-range/not-allowed (SelectableDates weekends) + DatePickerHeadline Input empty (Entered date); range Input DateInputValidator errors (format / year range / not-allowed / order) | [spec](https://m3.material.io/components/date-pickers/specs) |
